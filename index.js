...
// const PRODUCT_TABLE = "update_20250502_submodel";
const PRODUCT_TABLE = "shopify_product_lookup_20250530"; // For Shopify
// const PRODUCT_TABLE = "Products_20250212";
// const RELATIONAL_TABLE = "relations_product_20250507";
const RELATIONAL_TABLE = "shopify_relation_product_20250530"; // For Shopify
// const MODEL_TABLE = "Model";
// const MAKE_TABLE = "Make";
const MODEL_TABLE = "shopify_model"; // For Shopify
const MAKE_TABLE = "shopify_make"; // For Shopify
...
async function createRelationalTable() {
  ...
  while (hasMore) {
    let {
      ...
      .select("id, sku,make,model,year,type") // For Shopify
      .order("id", { ascending: true })
      .range(pageIndex * pageSize, (pageIndex + 1) * pageSize - 1);
    console.log(
      `Retrieving Products...(${pageIndex * pageSize}, ${
    ...
    if (products.length < pageSize) {
      console.log(
        "Products.length is less than page size, setting has more to false"
      );
...
async function prepareAndBatchInsertData(
  products,
  makes,
  models,
  years,
  ...
  for (const product of products) {
    ...
    // const [startYear, endYear] = product.year_generation
      const [startYear, endYear] = product.year // For Shopify
      .split("-")
      .map((year) => parseInt(year));
    if (!endYear) {
      const yearObj = years.find((y) => y.name === startYear.toString());
      if (!yearObj) {
        console.log("Couldn't find yearObj:", { yearObj, startYear });
      }
    ...
    for (let year = startYear; year <= endYear; year++) {
      ...
      if (yearObj) {
        ...
        if (inserts.length >= batchSize) {
          console.log(
            `Batch size has been reached, inserting: ${inserts.length} records`
          );
...
async function batchInsert(rows) {
  ...
  if (relationInsertError) {
    console.log("Error: ", relationInsertError);
    // ID : 14452
  }
}
// fetchAndUploadData();
